            ...
            <div class="content">
                <group>
                    ...
                    <div class="xin"><span>*</span>
                        <x-input label-width="105" title="姓 名" required v-model="username" class="pleft0" name="username"
                                 placeholder="請輸入姓名" type='text'></x-input>
                    </div>
                    <!--<div class="xin"><span>*</span>-->
                    ...
                    <x-input label-width="105" title="護 照" v-model="passport" class="pleft0" name="passport"
                             placeholder="請輸入護照號碼(選填)" type='text'></x-input>
                    <x-input label-width="105" title="通行證件號碼" v-model="permit_number" class="pleft0" name="permit_number"
                             placeholder="請輸入通行證件號碼(選填)" type='text'></x-input>
                    <x-input label-width="105" title="推薦會員編號" v-model="referee_uid" class="pleft0" name="referee_uid"
                             placeholder="請輸入推薦會員編號" type='number' :disabled="disabled" required></x-input>
...
<script>
  ...
  export default {
    ...
    mounted () {
      let that = this, uquid = document.cookie.indexOf('uquid=')
        that.getData()
      if(uquid==-1){
        that.disabled=false
      }else{
        that.referee_uid=cookie.get('uquid')
        that.disabled=true
      }
    },
    methods: {
        ...
        that.$api.H5UserRegister({
          ...
          .then(response => {
            ...
            if (response.data.state == 1) {
                that.u_mobilephone = data.mobile_phone
                  if(data.is_register==1){
                    that.$router.push('/UserHome')
                    cookie.set('user_isvip', 'yes', {expires: 30000, path: '/'})
                  }
            }
