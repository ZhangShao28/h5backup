...
<script>
  ...
  export default {
    ...
    mounted () {
      let quid = window.location.href.split('?quid=')[1];
      let that = this,  LT_state = cookie.get('loginType');
      ...
      if(quid){
        ...
        cookie.set('quid', quid, {expires: expiresTime, path: '/'})
        that.loginType=3
        that.switchText='返回登入頁'
        that.regdis_no=false
        cookie.set('loginType', '3', {path: '/'})
      }else{
        cookie.remove('loginType')
      }
      setTimeout(function () {
    ...
    methods: {
      ...
      loginBtn () {
        var that = this
//        that.$router.push('/MemberRule?cid=' + cookie.get('cid'))
