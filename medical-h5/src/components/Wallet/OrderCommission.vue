                        ...
                        <group>
                            <cell>
                                <span slot="title" class="listDiv">
                                    <div><span class="title">項目：</span>{{item.product_name}}({{item.level}})</div>
                                    <div><span class="title">訂單：</span>HK$ {{item.sale_money}}</div>
                                    <div v-if="item.yue_money!='0.00'"><span class="title">餘額抵扣：</span>HK$ {{item.yue_money}}</div>
                                    <div v-if="item.lower_level_name"><span class="title">下級：</span>{{item.lower_level_name}}</div>
                                </span>
                                <span slot class="listDiv">
                                    <div class="time">{{item.finish_time}}</div>
                                    <div class="moneyNum">HK$ {{item.common_money}}</div>
                                    <div v-if="item.yue_money!='0.00'">&nbsp;</div>
                                    <div v-if="item.lower_level_name">&nbsp;</div>
                                </span>
                            </cell>
                        </group>
                        <!--</router-link>-->
