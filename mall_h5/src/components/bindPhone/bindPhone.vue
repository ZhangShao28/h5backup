    ...
    <div id="login_page">
        ...
        <div class="content">
            ...
            <group class="weui-cells_form">
                <x-input title="中文姓名" v-model="u_username" type="text" placeholder="請輸入您的中文姓名" @on-blur="inputBlur"></x-input>
                <x-input title="英文姓名" v-model="u_username_en" type="text" placeholder="請輸入您的英文姓名" @on-blur="inputBlur"></x-input>
                <div class="birthDate">
                    <datetime
                            v-model="birth_date"
        ...
        <div class="login_btn">
            <x-button class="bigBtn" @click.native="loginBtn" :disabled="dis_no" text="綁    定"></x-button>
        </div>
    </div>
</template>
<script>
  import { XHeader, XInput, Group, XButton, Cell, Toast, cookie, CheckIcon,Datetime} from 'vux'
  export default {
    name: 'Login',
    components: {
      XHeader,
    ...
    methods: {
      ...
      loginBtn () {
        console.log( this.selvalue,this.username,this.inputValue,)
        var that = this
          if (that.inputValue == '') {
            that.$dialog.toast({
              mes: '手提電話不能為空！',
        ...
        if (that.username_en == ''&&that.username == '') {
          that.$dialog.toast({
            mes: '中文/英文姓名必須填寫一個',
            timeout: 1500,
            icon: 'error',
