<template>
    <div class="count_content">
        ...
        <div class="count_list">
            ...
            <div class="weui-cell count_title"><div class="shu"></div>用户统计</div>
            <div class="weui-cells vux-no-group-title countBox ">
                <div class="weui-cell">
                    <div class="total_con">
                        <div class="count_tit"><img src="../../../assets/Thetotalnumberofusers.png" alt="">总用户数(人)</div>
                        <div class="total_num">{{user_statistic}}</div>
                    </div>
                </div>
                <div  class="weui-cell count_con">
                    <div class="vux-cell-bd vux-cell-primary">
                        <div class="count_tit">今日用户数(人)</div>
                        <div class=" count_num">{{user_today}}</div>
                    </div>
                    <div class="fenge"></div>
                    <div class="vux-cell-bd vux-cell-primary">
                        <div class="count_tit">昨日用户数(人)</div>
                        <div class=" count_num">{{user_yesterday}}</div>
                    </div>
                </div>
            </div>
            <div class="weui-cell count_title"><div class="shu"></div>充电次数</div>
            <div class="weui-cells vux-no-group-title countBox ">
                <div class="weui-cell">
                    <div class="total_con">
                        <div class="count_tit"><img src="../../../assets/Totalchargingtimes.png" alt="">总充电次数(次)</div>
                        <div class="total_num">{{charging_num}}</div>
                    </div>
                </div>
                <div  class="weui-cell count_con">
                    <div class="vux-cell-bd vux-cell-primary">
                        <div class="count_tit">今日充电人数(次)</div>
                        <div class=" count_num">{{charging_today}}</div>
                    </div>
                    <div class="fenge"></div>
                    <div class="vux-cell-bd vux-cell-primary">
                        <div class="count_tit">昨日充电人数(次)</div>
                        <div class=" count_num">{{charging_yesterday}}</div>
                    </div>
                </div>
            </div>
        </div>
...
//                var xqid = this.selected;
                console.log("get",xqid)
                let myChart = this.$echarts.init(document.getElementById('myChart'))
                this.$http.post('/h5/merchant/statistic',this.$qs.stringify(
                    {
                        xqid:xqid
                    }
                )).then(response => {
                    ...
                    if(response.data.err==1){
                        var data = response.data.data.statistic
                        this.total_money=data.total_money
                        this.today_money=data.today_money
                        this.yesterday_money=data.yesterday_money
                        this.device_total_num=data.device_total_num
                        this.device_using_num=data.device_using_num
                        this.device_free_num=data.device_free_num
                        this.device_fault_num=data.device_fault_num
                        this.user_statistic=data.user_statistic
                        this.user_today=data.user_today
                        this.user_yesterday=data.user_yesterday
                        this.charging_num=data.charging_num
                        this.charging_today=data.charging_today
                        this.charging_yesterday=data.charging_yesterday
                        this.place_list=response.data.data.xq_list,
                            ...
                            series : [
                                {
                                    ...
                                    data:[
                                        {value:data.device_free_num, name:'未使用:'+data.device_free_num},
                                        {value:data.device_using_num, name:'使用中:'+data.device_using_num},
                                        {value:data.device_fault_num, name:'离线:'+data.device_fault_num},
                                    ],
                                }
                            ]
                        });
