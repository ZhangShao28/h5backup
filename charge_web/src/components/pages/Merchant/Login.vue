...
//            console.log(cookie.get('login_state_mer'))
            if(cookie.get('login_state_mer')==undefined){
...
//                this.$router.push('/Count')
                var that = this;
                that.$http.post('/h5/merchant/login', this.$qs.stringify(
                    {
                        mobile_phone:that.inputValue,
                    ...
                    .then(function (response) {
                        ...
                        if(response.data.err==1){
                            ...
                            cookie.set('login_state_mer', response.data.data.mer_token, {
                                expires: 30
                            })
                            that.$http.defaults.headers.mertoken =response.data.data.mer_token;
                            setTimeout(function () {
                                that.$router.push('/Count')
                            },2000)
                        }else{
                            that.$dialog.toast({
                                mes: response.data.msg,
